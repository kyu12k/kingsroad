...
> **보상은 승점이 아니라 시간이다** — 라는 것이 이 기능의 근거였다.
> 그래서 `getHardshipScoreScale()`이 이 세션에 **0**을 준다 — 이어지는 훈련 코스가 끝나면
> `stageClear`가 복습 승점을 정상 지급하므로, 여기서 또 주면 같은 복습에 두 번 주는 셈이다.

> ⚠️ **그런데 그 근거는 아직 검증되지 않았다 (2026-09-10).**
> 빠른 모드는 원래도 **2단계**(`[1,5]`)다. 승급되면 **백지(타이핑) + `[1]`**인데,
> 망각의 고난 실측이 구절당 32.8초라 **더 짧다는 보장이 없고**,
> 막히면 **백지 + `[1,5]`로 확실히 길어진다.**
> → `reviewSamples`로 실측 중. 아래 참조.
...
## 복습 소요 시간 표본 (`reviewSamples`, 2026-09-10)

**백지 승급이 정말 복습을 짧게 만드는가**를 판정하기 위한 기록. 위 ⚠️의 답을 여기서 낸다.

```js
reviewSamples = [ { s: '13-8', c: 2, ms: 41300, at: 1781... }, ... ]  // 최근 80개
```

| `c` | 뜻 |
|-----|-----|
| 0 | 전체 학습 `[1,2,3,4,5]` |
| 1 | 빠른 모드 **비승급** `[1,5]` ← 비교 기준 |
| 2 | 백지 승급 **통과** (백지 + `[1]`) |
| 3 | 백지 승급 **막힘** (백지 + `[1,5]`) |

**판정**: `c=2`의 중앙값이 `c=1`보다 뚜렷이 낮고, `c=3`의 비율이 낮아야 승급이 이득이다.
`c=3`이 많거나 `c=2`가 `c=1`보다 길면 **「보상은 시간」이라는 근거가 무너진다** —
남는 정당화는 '인출의 질'뿐인데 사용자가 체감하는 것은 시간이다.

### 구현에서 걸려 넘어질 곳

> ★ **`_beginReviewRun()`은 백지 승급 가로채기보다 먼저 불러야 한다.**
> 승급은 '백지 세션 → 훈련 코스'로 **두 번에 나뉘어** 도는데, 이어지는 코스(`quick-after-*`)에서
> 시작 시각을 다시 잡으면 **백지에 쓴 시간이 통째로 빠져 승급이 실제보다 유리하게 보인다.**
> 그래서 `quick-after-*`는 코스 표시(`c`)만 2/3으로 갱신하고 시작 시각은 건드리지 않는다.

- **구절 단위(`verseRecall`)가 아니라 표본 배열인 이유**: 승급된 구절은 앞으로 늘 승급되므로
  같은 구절 안에서 전/후 비교가 불가능하다. 승급된 구절과 아닌 구절을 **가로질러** 비교해야 하는데,
  승급되지 않은 구절에는 `verseRecall` 항목 자체가 없다. `verseRecall`을 404절 전체로 넓히면
  저장본이 그만큼 커지므로(동기화 사고 이력이 있는 문서다) 별도 고리버퍼로 뒀다
- **1초 미만·15분 초과는 버린다** — 오작동이거나 자리를 비운 것이라 소요 시간이 아니다
- **`type === 'normal'`만** 기록한다. 중간점검·보스전은 단위가 달라 섞으면 분포가 무의미해진다
  (집중 훈련은 `stageClear`의 `isTrainingMode` 방어막에서 이미 걸러진다)
- **병합은 `_mergeReviewSamples()`로 합집합** — `_mergeVerseRecall`처럼 한쪽을 통째로 고르면
  다른 기기에서 잰 표본이 사라진다. `at`으로 중복 제거 후 시간순 `slice(-80)`
...
| **보스전 빈칸·백지** | **한 장 — 망각의 고난과 분량·위험이 같다** | **1.0** |
...
**`_blankScoreAlreadyToday()` — 같은 구절을 *같은 콘텐츠로* 다시 하면 하루 1회 제한**
(오전 6시 경계, `verseRecall[].scoredBy[kind]`). 배율만으로는 부족했다 —
체력 47이면 1구절 47점인데 30번 반복하면 1,410점이 된다.

> ★ **콘텐츠를 구분하지 않으면 정상적인 학습 순서를 벌하게 된다.** (2026-09-11 수정)
> 처음에는 구절 단위로만 셌다(`lastScoredAt`). 그랬더니
> **구절을 하나씩 복습하며 결과 화면에서 백지로 확인하고, 그 구간의 중간점검을 백지로 이어서 하는 것**
> — 우리가 권하는 바로 그 순서가 **중간점검 전 구절 0점**이 됐다.
> 어뷰징을 막으려다 복습을 막은 셈. 막아야 하는 것은 **같은 중간점검의 반복**이지 다른 콘텐츠가 아니다.

`_blankScoreKind()`가 콘텐츠를 가른다:

| 콘텐츠 | `kind` | 제한 |
|--------|--------|------|
| 중간점검 빈칸·백지 | `'mid'` | 구절별 하루 1회 |
| 보스전 빈칸·백지 | `'boss'` | 구절별 하루 1회 (`'mid'`와 별개) |
| 결과 화면 백지 확인 | `'vc'` | 구절별 하루 1회 (`'mid'`와 **별개로 센다**) |
| 망각의 고난 | — | **제한 없음** (한 장 진입 비용이 이미 억제 장치) |
| 빠른 모드 승급 | — | 제한 없음 (배율 0이라 무관) |

- **기록은 반복해도 계속 쌓인다** — 막는 것은 승점뿐. 측정 데이터가 줄면 이 기능의 목적이 사라진다
- `lastScoredAt`은 표시·분석용으로만 남긴다. **판정은 `scoredBy`가 한다**
- 제한에 걸렸을 때 문구는 `hardship_feedback_correct_scored_today`
  (*"오늘 이미 받은 승점"*) — 예전의 *"승점 없음"*은 왜 못 받는지를 말해주지 않았다.
  `rewardBlocked`(암송의 고난, 그 장을 오늘 이미 완료)와는 다른 사유라 문구를 나눴다
...
> **메뉴가 아니라 흐름 안에 둔 이유**: 사용자는 **찾아가야 하는 것을 안 쓴다.**
> (아래 「무엇이 실제로 쓰이는가」 참조 — 강제로 마주치는 선택은 과반이 바꾸지만,
...
## 중간점검·보스전 빈칸·백지 (2026-09-08 / 보스전 2026-09-11)

난이도가 4칸이다. **중간점검과 보스전 양쪽**에 빈칸·백지가 있다.

> 처음에는 *"보스전은 한 장 최대 29절이라 타이핑 부담이 크다"*며 중간점검에만 뒀는데,
> **망각의 고난이 이미 똑같은 29절을 타이핑시키고 있었다**(32명이 하고 정답률 96.5%).
> 부담이 진짜 이유였다면 고난도 아무도 못 했을 것이다. 근거가 성립하지 않아 2026-09-11에 열었다.
> **전제조건이던 고난 이어하기를 먼저 넣었다** — 16분짜리가 전화 한 통에 사라지면 안 된다.
...
> ★ **보스전 빈칸·백지(`bossStageId`)는 예외로 저장한다.** 최대 29절·16분짜리라
> 이어하기가 **가장 필요한 곳**이고, 후속 흐름(`stageClear('boss')`)은 `'completed'`에서만 돌아
> 중간 복원과 충돌하지 않는다. 체크포인트에 `kind`(`'boss:{id}'` / `'free'`)를 함께 저장해
> **그 장의 망각의 고난과 구분한다** — 구절 묶음이 완전히 같아서 kind가 없으면 서로 이어져 버린다.

**그 밖에 빌려 쓰는 세션은 저장하지 않는다** — 중간점검 빈칸·결과 화면 백지 확인·빠른 모드 승급.
...
그래서 사람들은 기억 상태가 아니라 **부담**으로 골랐다 — 보스전 경험자 72명 중 54%가 '보통'인데,
29절을 초성으로 하는 것이 버거워 내려간 것이지 자기 기억 상태에 맞아서가 아니다.

중간점검 설정 모달의 각 난이도 칸 아래에 한 줄을 붙인다.
...
규칙은 `추천 = 등급 + 1`. 한 칸 위가 🔥, 두 칸 이상 위가 🔒, 아래는 전부 😌.
...
> **복습 스텝·클리어 횟수를 쓰지 않는 이유**: '화면에 답이 있는 상태에서 재구성한 횟수'라
> 단서 없이 나오는지를 증명하지 못한다. 쉬운 성공의 반복은 오히려 과신을 키운다.

> ★ **기억 강도를 일부러 뺐다.** 시간이 지나면 떨어지는 값이라 이걸 쓰면
> *"오래됐으니 쉽게 해드릴게요"*가 되는데, **가물가물할 때가 인출 연습의 가치가 가장 큰 순간**이다.
> 정확히 거꾸로 쓰는 셈이다. 반면 `lastOk === false`는 시간 추정이 아니라 **직접 관찰한 실패**라 쓴다.

- `HINT_OK_RATIO = 0.2`는 **잠정값**(미확정). 라벨은 **보상을 한 푼도 바꾸지 않으므로**
  틀려도 손해가 '권유가 조금 어긋남'뿐이다 — 그래서 데이터를 기다리지 않고 먼저 넣었다
- `lastVerseLen`이 없는 옛 기록(2026-09-10 이전)은 길이를 모르므로 **힌트 0일 때만** 등급 2로 인정

### 구간으로 합칠 때 — **가장 약한 고리**

`_getMidBossRecallInfo()`가 소속 구절 등급의 **최솟값**을 쓴다.

> 중간점검은 3~4절뿐이라 한 절이 막히면 거기서 전체가 멈춘다. 평균을 쓰면 그 한 절이 숨어
> **"알맞다고 해서 갔는데 벽이었다"**가 된다.

**보스전(한 장)은 반대로 비율을 쓴다** (`_getBossRecallInfo`) — 최대 29절이라 최솟값을 쓰면
하나만 약해도 **영원히 0에 머물러** 표시가 무의미해진다.

| 조건 | 등급 |
|------|------|
| 등급 2인 절이 **8할 이상** | 2 (백지 ⭐) |
| 등급 1 이상인 절이 **절반 이상** | 1 (빈칸 ⭐) |
| 그 아래 | 0 (어려움 ⭐) |

### 그 밖에

- **🔒는 잠그지 않는다.** 누르면 들어가진다 — 힌트 잠금 때와 같은 결론(안내지 금지가 아니다)
- **보스전에도 붙는다**(2026-09-11, 빈칸·백지가 열리면서). 다만 **집계 방식이 다르다** — 아래 참조
- **근거 한 줄**(`.bso-fit-note`)을 함께 보여준다 — *"이 구간 4절 중 1절을 백지로 써봤어요"*.
  라벨만 있으면 마법처럼 보이고, **무엇을 하면 등급이 오르는지**가 안 보인다
...
## 태그(#번호) 발급 — '0000'에 갇히던 버그 (2026-09-10)

**`'0000'`은 '태그 없음'을 뜻하는 자리표시자다.** 저장본에 `tag`가 없으면
[game.js:2001](game.js#L2001) `myTag = String(parsed.tag || "0000")`이 이 값을 채운다.

그런데 태그를 만들어주는 유일한 지점(`confirmProfile`)의 조건이 `if (!myTag)`였다.
**`'0000'`은 truthy라 한 번 박히면 영원히 걸리지 않는다.**

> 결과: **프로필(지파·이름)을 끝까지 채운 사람도 번호를 못 받은 채 남았다.**
> 프로필을 다시 저장해도 마찬가지다. 친구 추가를 누르면 *"닉네임을 먼저 설정해주세요"*
> ([game.js:14505](game.js#L14505), [24471](game.js#L24471))가 뜨는데 닉네임은 이미 있다 —
> 사용자 입장에서는 무엇을 하라는 건지 알 수 없는 상태였다.

**2026-09-10 실측** (`saves` 6,917개 전수):

| 구분 | 개수 | 암기 진도 보유 |
|------|------|----------------|
| 태그 정상 | 2,997 | 851 |
| `tag='0000'` — **닉네임 있음(버그 피해자)** | 332 | **127** |
| `tag='0000'` — 닉네임 '순례자'(프로필 미완료) | 1,225 | 125 |
| `tag=''` (빈 껍데기, 96%가 완전히 빔) | 2,363 | 5 |

피해자 127명 중 25명이 최근 30일 활동, 19명이 20절 이상. **번호가 없으면 복구 수단도 없어서
브라우저를 정리하면 진도가 통째로 사라진다.**

### 고친 것

- `confirmProfile`: `if (!myTag || myTag === '0000')`
- `ensureTagAssigned()` — 이미 갇힌 사람은 프로필을 다시 열지 않으므로 **접속 시 자동 발급**한다.
  `initFirestoreSync()`를 얇은 래퍼로 감싸 **동기화가 끝난 뒤** 호출한다
  (본체 `_initFirestoreSyncCore()`는 반환 지점이 여러 곳이라 안쪽에 넣으면 경로마다 빠뜨린다)

> **닉네임이 '순례자'면 발급하지 않는다.** 프로필을 만든 적 없는 사람을 조용히 랭킹에 올리는 것이 된다.
> 그쪽(1,225명)은 안내를 띄워 직접 정하게 하는 별도 작업의 몫이다.

> **순서가 중요하다: `saveGameData()` → `await syncToFirestore()` → `saveMyScoreToServer()`.**
> `saves/{uid}.tag`가 먼저 올라가야 `submitScoreSecure`의 `verifyTag()`와
> leaderboard 규칙의 소유권 검사가 새 태그를 인정한다.

- 예상 부작용: 구제된 사람들이 **그동안 쌓은 점수를 들고 랭킹에 새로 나타난다.** 정당한 점수지만
  순위가 흔들린다. 돌아오는 사람만 순차 적용되므로 한꺼번에 오지는 않는다

### 프로필 안내 (`maybeShowProfileNudge`, 2026-09-10)

닉네임이 `'순례자'`인 채(= 프로필을 만든 적 없음) 외우고 있는 사람에게 **주 1회** 이름을 정하도록 권한다.
대상은 1,225명 중 **진도를 가진 125명**(5절 이상 44명).

| 조건 | 값 |
|------|-----|
| 태그 | 없음 또는 `'0000'` |
| 닉네임 | `'순례자'` (있으면 대상 아님 — `ensureTagAssigned()`가 조용히 해결) |
| 진도 | `_getClearedVerseCount() >= 1` |
| 주기 | `getMissionPointWeekId()` (월요일 오전 6시 경계), `kingsRoad_profileNudgeWeek` |

> **접속 직후가 아니라 '한 구절 클리어 직후'에 띄운다** — `closeResultModal`의 일반 클리어 경로,
> `tryShowMilestone` 0.5초 뒤의 1.4초 지점. 방금 무언가를 얻은 순간이라 "이걸 지키자"가 설득력을 갖는다.
> 접속 직후는 아직 잃을 것이 없어 그냥 닫히는 광고가 된다.

> **막아 세우지 않는다.** 「나중에」·바깥 클릭 모두 닫히고 다음 주에 다시 묻는다.
> 강제하면 '그냥 시작할 수 있다'는 게스트의 장점 자체가 사라진다.
> 표시 기록은 **띄우는 시점에** 남긴다 — 어떻게 닫든 이번 주엔 다시 묻지 않는다.

- 업적 팝업(`isMilestoneShowing`)이나 다른 `.modal-overlay.active`가 있으면 2.5초씩 최대 4회 양보한다.
  `#stage-sheet`는 `modal-overlay`가 아니라 이 검사에 걸리지 않는다(시트 위에 떠도 된다)
- 문구는 혜택보다 **잃을 것**을 먼저 말한다 — "이 기록은 지금 쓰는 브라우저에만 있어요"
- localStorage를 못 읽으면 **묻지 않는다**. 기억할 수 없는 상태에서 묻기 시작하면 매 클리어마다 뜬다

> **`weeklyCleanupGhostAccounts`(functions/index.js 끝, 미배포)는 폐기 대상.**
> 조건(`leaderboard`에서 `totalScore==0` + 7일 미접속)에 걸리는 건 17개뿐이고,
> 필요한 복합 색인이 없어 실행 자체가 실패한다. 유령은 `leaderboard`(939개)가 아니라 `saves`에 있고,
> 예전에 "4,108개 중복"으로 본 것은 중복이 아니라 **무태그 껍데기**였다
> (실제 중복은 16개 태그/잉여 75개뿐 — `cleanupOldSaveDocs`가 돌고 있다).

---

## 무엇이 실제로 쓰이는가 (2026-09-10 실측)

> ⚠️ **"87.4%가 기본값을 그대로 쓴다 = 기본값 관성"은 잘못된 해석이었다.**
> 그 분모에 **난이도 모달을 한 번도 열어본 적 없는 사람**이 전부 들어 있었다.
> 실제로 그 선택에 마주친 사람만 보면 정반대다.

**30일 활성 551명** (`saves`에서 `tag`가 `''`·`'0000'`이 아닌 문서만 — 빈 껍데기가 분모를 오염시킨다)

| 난이도 설정 | 보통 | 어려움 |
|-------------|-----:|-------:|
| 활성 전체 551명 | 11.4% | 87.3% |
| **중간점검을 해본 120명** | **40.0%** | 60.0% |
| **보스전을 해본 72명** | **54.2%** | 45.8% |

**보스전 경험자는 과반이 난이도를 바꾼다.** 관성이 아니다 — 한 장 29절을 초성으로 하는 것이 버거워
단어 버튼으로 내려간다. 사람들은 선택을 한다. 다만 기준이 **'내 기억 상태'가 아니라 '부담'**이다.

### 도달 깔때기

| | 인원 | 전체 | 한 절이라도 깬 267명 중 |
|---|---:|---:|---:|
| 1절 이상 | 267 | 48.5% | 100% |
| 10절 이상 | 163 | 29.6% | 61.0% |
| 중간점검 | 120 | 21.8% | 44.9% |
| 보스전 | 72 | 13.1% | 27.0% |
| 주소의 고난 | 45 | 8.2% | 16.9% |
| 망각의 고난 | 32 | 5.8% | 12.0% |
| 구절의 고난 | 27 | 4.9% | 10.1% |
| 암송의 고난 | 12 | 2.2% | 4.5% |
| 백지 경험(`verseRecall`) | 12 | 2.2% | 4.5% |
| **고난 4종 모두** | **9** | 1.6% | **3.4%** |

- **빈칸·백지를 마지막으로 쓴 사람 0명** (중간점검 경험자 120명 중). 단 배포 이틀째이고,
  보스전 진입 시 `bossDifficultyMode`가 `'hard'`로 되돌아가는 코드([game.js](game.js) `openBossSetupModal`)가
  흔적을 지웠을 수 있다
- **`bossOrderMode` 무작위는 6~8%** — 이쪽은 실제로 거의 안 바뀐다

### 진짜 구분선은 의지가 아니라 **배치**다

| 어디에 있나 | 실제 이용 |
|-------------|-----------|
| **매번 강제로 뜨는 모달** (난이도) | 과반이 바꿈 |
| **흐름 안의 버튼** (결과 화면 백지 확인) | 이틀 만에 진행자의 4.5% |
| **찾아가야 하는 메뉴** (고난 길) | 4종 다 써본 사람 3.4% |

> 새 콘텐츠를 만들 때 물어야 할 것은 "사람들이 할까?"가 아니라
> **"강제로 마주치는가, 흐름에 있는가, 찾아가야 하는가?"** 이다.
...
**힌트는 막힌 지점부터 순서대로 열린다**(`getHardshipMemoryHintPlan`). 따라서 `revealedHints[0]`이
그 시도에서 **처음 막힌 글자 위치**다. `-1`은 힌트를 쓰지 않았다는 뜻.

> **개수(`hints`)만으로는 "머리에서 막혔다"와 "군데군데 막혔다"를 구분할 수 없다.**
> 구절 암송은 사슬 구조라 각 마디가 다음 마디의 단서가 되는데 **첫 마디만은 앞에 아무것도 없다**
> (유일한 단서가 주소다). 백지 실패가 '구절을 모른다'가 아니라 **'주소 → 첫 마디' 연결이 약한 것**이라면,
> 처방은 전체를 쉽게 하는 것이 아니라 **첫 마디만 세우는 단계**가 된다.

- `lastFirstHintIdx / lastVerseLen`이 **0~0.1에 몰리면** 시동 가설이 맞다 → 첫 마디 단계를 만든다
- **흩어지면** 시동이 아니라 전체 정착이 덜 된 것 → 다른 처방
- `lastVerseLen`을 함께 남기는 이유: 짧은 구절에서는 index 2도 중간일 수 있어 **비율로 봐야** 한다
- `memory`/`learn`(타이핑)에서만 남긴다 — `endurance`는 음성이라 힌트 개념이 없다

> **방향 주의** — 이 문제에 쓸 수 있는 모드는 정해져 있다.
> | 모드 | 방향 | 백지에 도움 |
> |------|------|-------------|
> | 주소의 고난 | 구절 전체 → 주소 | **역방향** (전이가 약하다) |
> | 구절의 고난 | 주소 → 구절 고르기 | **정방향·재인** ← 중간 단계 후보 |
> | 백지 | 주소 → 구절 산출 | 목표 |
>
> 인출 연습은 **연습한 방향으로만 강하게 남는다.** 주소의 고난을 중간에 끼워도 시동 문제는 잘 풀리지 않는다
> (그건 '지금 몇 장 몇 절인가'를 아는 별개의 기술이다).
> 구절의 고난은 방향이 맞지만 **선택지로 구절 전체를 보여줘서** 백지 직전에 두면 정답을 읽혀준다.
> 쓰려면 선택지를 **첫 마디만** 잘라야 한다. 미착수 — 위 데이터를 보고 결정한다.
