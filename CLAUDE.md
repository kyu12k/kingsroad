...
**힌트 버튼은 헤더가 아니라 입력 보드 아래 액션 행에 있다** (`#hardship-inline-hint-btn`).
타이핑 중 시선은 입력 보드에 있어서 헤더 구석의 칩은 눈에 안 들어오고,
정작 「모르겠어요」는 보드 아래 있었다. **회복 수단을 포기 수단보다 앞에** 놓는다.
`armHardshipHintNudge()`가 **8초간 입력이 없으면 버튼을 은은하게 맥동**시킨다 —
