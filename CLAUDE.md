...
## 베타 스위치 (2026-09-13 → 2026-09-14 전원 적용)

> **2026-09-14: `_BETA = true` 고정.** 밭·단비가 전원에게 나갔다. 옛 경로(체력 감소·부활·상점·떡·방패·부스터 구매)는
> 코드에 남아 `_BETA` 분기로만 죽어 있다. 한 주 뒤 「확인 후 정리할 것」대로 지운다.
> 되돌리려면 그 한 줄을 `localStorage` 판독으로 바꾸면 전원이 옛 동작으로 돌아간다.
> 떡·방패 환급(`itemRefund`, 50젬)은 로드 시 1회 — `loadGameData` 끝, 표식이 저장본에 실린다.

원래 방식: `localStorage.kingsRoad_beta = '1'`이면 `_BETA = true`. **테스트 서버 대신** — 배포는 전원에게 나가고,
켠 기기에서만 새 기능이 보인다. 확인되면 조건을 지우고 한 번 더 배포한다.

- `isBeta()` / 모듈 상수 `_BETA` (부팅 시 1회 읽음 — `t()`가 뜨거운 경로라 매번 localStorage를 읽지 않는다)
- **`t()` 훅**: 베타면 `key + '__field'` 변형이 있을 때 그것을 쓴다. 기존 UI 문구를 화면마다 분기하지 않고
  i18n 한 곳에서 바꾸기 위해서다. 확인되면 `__field` 문구를 본 키로 옮기고 훅을 지운다
...
| | 동작 |
|---|---|
| **오답 → 체력 -1** | **없음.** `_loseHeart()` 8곳이 베타에서 no-op. 오답의 대가는 그 구절 0점(백지) / 다시 풀기(초성·단어) |
| 부활 모달·고난 `'hearts'` 종료 | 체력이 0에 닿지 않으므로 자연히 죽음 |
| Step 4 오답 연출 (`showDamageEffect`) | 💔 → **❌**. 잃는 게 없으니 '틀렸다'만. 흔들림·오답음은 그대로 |
| 생명의 떡 · 믿음의 방패 | `useBattleItem`/`_tryUseShield`가 no-op. 헤더 칩 숨김 |
| 헤더 표시 | ❤️ 47/47 → **🌾 밭 47** (고정값이라 `/최대` 없음) |
| 상점 | `openShop()` → `openFieldScreen()`. 지도 헤더의 🌾 칩이 입구 |
...
- `_loseHeart()` 호출 8곳 삭제, 함수 삭제
- `__field` i18n을 본 키로, `t()` 훅 삭제
- 부활 모달 HTML·`showReviveModal`·`revivePlayer` 삭제
- 상점 화면(`#shop-screen`)·`SHOP_ITEMS` 전체·`buyItem`·`activateBooster` 삭제.
  **부스터 구매도 없앤다** — 단비가 대체한다(아래 「단비」). 30일 활성 581명 중 부스터 사용자 1명
- 떡·방패 재고 환급: 30일 활성 97명 보유(떡 2,796·방패 162), 50젬씩 총 14.8만 젬. 첫 접속 때 1회, 토스트로 알리고 재고 0
- Step 4 오답 연출 `showDamageEffect`의 `_BETA` 분기 → ❌만 남김
- 더보기 메뉴 `⛺` → `🌾`, `nav_shop` 문구
- 기존 떡·방패 재고 — 50젬씩 환급할지 결정
- CLAUDE.md의 「고난 길 승점 계산」 등 "체력" 서술을 밭으로
...
| **실패(체력 소진) 결과 화면** (`#btn-easier-boss`) | 순수 망각의 고난 + `forcedChapter` 있을 때. **방금 벽에 부딪힌 순간** |
...
> **2026-09-16: 모든 힌트가 무료이고 언제나 열려 있다.** `getCurrentHintCost()`는 0, `isHintUnlocked()`는 고난에서
> 타이핑 힌트가 있는지만 본다. 아래 "먼저 시도, 그다음 도움"의 잠금(`hintAttemptMade`)과 💎10은 **폐기**.
> 근거: 힌트로 전문을 읽어도 다음 판엔 힌트가 없으니 결국 스스로 꺼내야 하고, 시도를 막아 "너무 어려워서 안 하게 되는"
> 손해가 베끼기의 손해보다 훨씬 크다. 집중 훈련 잠금은 이유 없이 일괄 규칙에 딸려 들어간 것이었다.
> `markHintAttempt`/`resetHintLock`/`.hint-locked`는 호출처가 남아 있으나 더 이상 아무 효과가 없다 — 정리 대상.
...
- 잠금(`hint-locked`)과 다르다 — 잠금은 '먼저 시도', 간격은 '연타 금지'. 타이핑 백지에는 잠금이 없고 간격만 있다
